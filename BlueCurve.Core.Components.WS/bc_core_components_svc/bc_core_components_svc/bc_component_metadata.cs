    ...
    //k  
{
    ...
    public class bc_component_metadata
    {
        ...
        public void set_metadata(bc_component_mode mode, bc_core_components_svc.bc_core_component_parameters parameters)
        {
            try
            {
                object res;
                ...
                if (ares.GetUpperBound(0) == 6)
                {
                    ...
                    if (mode == bc_component_mode.COMPONENT)
                    {
                        ...
                        if (parameters != null)
                        {
                            ...
                            if (parameters.lparameters.Count != param_count)
                            {
                                _certificate.error_state = true;
                                _certificate.server_errors.Add("Parameter Count Mismatch for type: " + _rc.type.ToString() + " : " + param_count.ToString() + "/" + parameters.lparameters.Count.ToString());
                            }
                        }
                    }
