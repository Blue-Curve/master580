...
#End Region
        ...
        Try
            ...
            For i = 0 To bc_am_workflow.docs.document.Count - 1
                ...
                If bc_cs_central_settings.generic_filter = False Then
                    bc_am_workflow.lauthor.Add(CInt(bc_am_workflow.docs.document(i).originating_author))
                Else
                    bc_am_workflow.lauthor.Add(bc_am_workflow.docs.document(i).summary)
                End If
                ...
                If (mode = 0 Or (mode = 1 And bc_am_workflow.docs.document(i).unread = True) Or (mode = 3 And bc_am_workflow.docs.document(i).stage_expire_date < Now) Or (mode = 2 And (Now.AddDays(bc_am_workflow.pre_expire_alert_notify) > bc_am_workflow.docs.document(i).stage_expire_date) And Now < bc_am_workflow.docs.document(i).stage_expire_date) Or (mode = 4 And bc_am_workflow.docs.document(i).stage_change_flag = True)) And ((bc_am_workflow.matches_only = True And bc_am_workflow.docs.document(i).search_flag = True) Or bc_am_workflow.matches_only = False) Then
                    ...
                    If bc_am_workflow.sauthor_id.Count > 0 Then
                        ...
                        Else
                            REM PR
                            If in_author_filter(bc_am_workflow.docs.document(i).summary) = False Then
                                exclude = True
                            End If
                        End If
                    End If
                    ...
                    If exclude = False Then
                        ...
                        If bc_am_workflow.docs.document(i).id = 0 Then
                            oitem.SubItems.Add(CStr(bc_am_workflow.docs.document(i).filename))
                        Else
            ...
            If Len(bc_am_workflow.selected_doc_id) <> 0 Then
    ...
    Private Sub DocumentList_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DocumentList.MouseUp
        ...
        Try
            ...
            For i = 0 To Me.DocumentList.Items.Count - 1
                ...
                If Me.DocumentList.Items(i).SubItems(1).Text = bc_am_workflow.selected_doc_id Then
                    ...
                    For j = 0 To bc_am_workflow.docs.document.Count - 1
                        REM steve wooderson 08/07/2013 local docs
                        If CStr(bc_am_workflow.docs.document(j).id) = bc_am_workflow.selected_doc_id _
                         Or bc_am_workflow.docs.document(j).filename = bc_am_workflow.selected_doc_id Then
            ...
            For i = 0 To bc_am_workflow.docs.document.Count - 1
                REM steve wooderson 08/07/2013 local docs
                If CStr(bc_am_workflow.docs.document(i).id) = Me.DocumentList.SelectedItems(0).SubItems(1).Text _
                    Or bc_am_workflow.docs.document(i).filename = Me.DocumentList.SelectedItems(0).SubItems(1).Text Then
                    ...
                    Else
                        ...
                        For l = 0 To bc_am_workflow.oopen_doc.onetwork_docs.document.Count - 1
                            If Me.DocumentList.SelectedItems(0).SubItems(1).Text = CStr(bc_am_workflow.oopen_doc.onetwork_docs.document(l).id) _
                            Or Me.DocumentList.SelectedItems(0).SubItems(1).Text = bc_am_workflow.oopen_doc.onetwork_docs.document(l).filename Then
                                tdoc = bc_am_workflow.oopen_doc.onetwork_docs.document(l)
                                tdoc.local_filename = bc_am_workflow.oopen_doc.onetwork_docs.document(l).filename
                                tdoc.local_index = bc_am_workflow.oopen_doc.onetwork_docs.document(l).id
                                tdoc.master_flag = True
                                tdoc.arrive = bc_am_workflow.oopen_doc.onetwork_docs.document(l).arrive
                                tdoc.acknowledged = bc_am_workflow.oopen_doc.onetwork_docs.document(l).acknowledged
                                Exit For
                            End If
                        Next l
                        REM tdoc.filename = Me.DocumentList.SelectedItems(0).SubItems(1).Text
                    End If
    ...
    Private Sub DocumentList_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DocumentList.SelectedIndexChanged
