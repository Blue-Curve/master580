...
Public Class bc_dx_am_behalf_author
    ...
    Public Sub load_data(ByVal users As ArrayList) Implements Ibc_am_behalf_author.load_data
        For i = 0 To users.Count - 1
            Me.ComboBox1.Properties.Items.Add(users(i))
        Next
...
Public Class Cbc_am_behalf_author
    ...
    End Function
    Public Sub save_originator(ldoc_id As String)
        Try
            Dim sql As New bc_om_sql("exec dbo.bc_core_set_originator_for_doc '" + CStr(ldoc_id) + "'," + CStr(originator))
            If bc_cs_central_settings.selected_conn_method = bc_cs_central_settings.ADO Then
                sql.db_read()
