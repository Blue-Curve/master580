...
Public Class bc_ao_in_excel
                ...
                For i = col To MAX_COLS
                    Try
                        ...
                        Else
                            ...
                            If pcode = 0 Then
                                create_row_value_formula(bc_year + bc_e_a_value, bc_period, activesheet, value_row, i, row, 0, year_formula, e_a_value_formula, ped_formula, bc_acc, acc_formula, "")
                            Else
                        ...
                        For k = 0 To .rows.count - 1
                            If .rows(k).link_found = True Then
                                ...
                                For l = 0 To .rows(k).values_list.count - 1
                                    ...
                                    tmp_header = New tmp_period_header(.rows(k).values_list(l).year, .rows(k).values_list(l).period_id, .rows(k).values_list(l).accounting_standard, .rows(k).values_list(l).year_formula, .rows(k).values_list(l).e_a_formula, .rows(k).values_list(l).period_id_formula, .rows(k).values_list(l).acc_formula, tmp_headers)
                                Next
                            End If
                        Next
                    End With
    ...
    Public Sub bc_array_excel_export(ByVal exportlist As Array)

        Dim row As Long
        Dim col As Long

        exceldoc.activesheet.cells(1, 1).value = "Aggregation Audit Information"
    ...
    Public Sub open_addins(ByVal oxlApp As Object)
    ...
    Private Class tmp_period_header
        ...
        Public Sub New(ByVal year As String, ByVal period As String, ByVal acc As String, ByVal year_formula As String, ByVal e_a_formula As String, ByVal period_formula As String, ByVal acc_formula As String, ByRef headers As tmp_period_headers)
            Dim i As Integer
    ...
    Public Function populate_object_with_data(ByVal output_only As Boolean) As Boolean
        ...
        Try
            ...
            If validateheaderdata(headervalues) = False Then
                ...
                'Dim omessage As New bc_cs_message("BlueCurve Insight", "Header Validation Failed Please Check cell Comments and Correct.", bc_cs_message.MESSAGE)
                Dim bc_am_in_main_frm As New bc_am_in_main_frm
                If cell_errors.bc_om_cell_errors.Count > 0 Then
                    bc_am_in_main_frm.lstcontributor.visible = False
                    Dim oapi As New API
                    'API.SetWindowPos(bc_am_in_main_frm.Handle.ToInt32, API.HWND_TOPMOST, 10, 10, 1, 1, 1)
                    bc_am_in_main_frm.Show()
                    bc_am_in_main_frm.TopMost = True
                End If
                bc_am_in_main_frm.populate_errors_list(cell_errors)
            Else
