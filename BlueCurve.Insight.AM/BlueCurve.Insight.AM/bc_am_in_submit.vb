                ...
                If oexcel.populate_object_with_data(False) = True Then
                    ...
                    If bc_am_insight_formats.controlled_Submission = 1 Then
                        ...
                        If oexcel.save(bc_cs_central_settings.local_repos_path + dfilename, False) = False Then
                            omessage = New bc_cs_message("Blue Curve - insight", "Workbook cannot be saved so submission aborted!", bc_cs_message.MESSAGE)
                    ...
                    If bc_cs_central_settings.selected_conn_method = bc_cs_central_settings.ADO And frminsightsubmit.close_mode <> bc_am_in_main_frm.LOCAL Then
                    ...
                    End If
                    If bc_cs_central_settings.selected_conn_method = bc_cs_central_settings.SOAP And frminsightsubmit.close_mode <> bc_am_in_main_frm.LOCAL Then
                        Dim gbc As New bc_cs_db_services(False)
        ...
        Finally
            ...
            ElseIf server_success = True And warningstype.Count > 0 Then
                ...
                If omessage.cancel_selected = False Then
                    Dim ofrmwarnings As New bc_am_submit_warnings
                    ofrmwarnings.oexcel = oexcel
                    Dim lvw As ListViewItem
                    For i = 0 To warningstype.Count - 1
                        lvw = New ListViewItem(CStr(warningssheet(i)), 0)
                        lvw.SubItems.Add(CStr(warningstype(i)))
                        lvw.SubItems.Add(CStr(warningstx(i)))
                        If warningstype(i) = "Data" Then
                            lvw.SubItems.Add(CStr(warningsrow(i)))
                            lvw.SubItems.Add(CStr(warningscol(i)))
                        End If
                        ofrmwarnings.Lvwarnings.Items.Add(lvw)
                        Dim oapi As New API
                        'API.SetWindowPos(ofrmwarnings.Handle.ToInt32, API.HWND_TOPMOST, 10, 10, 1, 1, 1)
                        ofrmwarnings.Show()
                        ofrmwarnings.TopMost = True
                    Next
                End If
            End If
