...
#End Region
    ...
    Public Shared lib_list As New ArrayList
    ...
    Public Sub load_start(Optional ByVal template_only As Boolean = False)
        Try
            ...
            For i = 0 To insight_items.libray_calcs.libray_calculations.Count - 1
                lib_list.Add(insight_items.libray_calcs.libray_calculations(i))
            Next
    ...
    Private Sub ctemplate_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctemplate.SelectedIndexChanged
        ...
        Try
            ...
            If change = True Then
                Dim omsg As New bc_cs_message("Blue Curve", "Changes have occurred to the previously selected template, continuing will discard these changes." & vbCrLf & "Do you wish to continue?", bc_cs_message.MESSAGE, True, False, "Yes", "No", True)
    ...
    Private Sub load_item_attributes(ByVal mode As Boolean)
        Try
            ...
            fitems.ccurrtype.Items.Add("Non Monetary")
            fitems.ccurrtype.Items.Add("Reporting")
            fitems.ccurrtype.Items.Add("Trading")
            fitems.ccurrtype.Items.Add("Reporting Denom")
            fitems.ccurrtype.Items.Add("Trading Denom")
            fitems.ccurrtype.SelectedIndex = 1
            ...
            If mode = True Then
                ...
                If Me.selsec < osheet.bc_om_insightsections.Count Then
                    With osheet.bc_om_insightsections(Me.selsec).rows(Me.litems.SelectedItems(0).Index)
                        ...
                        fitems.ccurrtype.SelectedIndex = .monetary
                        ...
                        For j = 0 To osheet.validations_ids.Count - 1
                            Try
                                fitems.lval.Items(j).SubItems(1).Text = "none"
                            Catch
                            ...
                            For i = 0 To .validations.bc_om_cell_validation.count - 1
                                If osheet.validations_ids(j) = .validations.bc_om_cell_validation(i).validation_id Then
                                    ...
                                    fitems.lval.Items(j).SubItems(1).Text = "error"
                                    If osheet.validations_ids(j) = 7 Then
                                        fitems.lval.Items(j).SubItems(1) = Me.litems.SelectedItems(0).SubItems(8)
                                        fitems.lval.Items(j).SubItems(1).Text = "set"
                                    End If
                                End If
                            Next
                            For i = 0 To .warnings.bc_om_cell_validation.count - 1
                                If osheet.validations_ids(j) = .warnings.bc_om_cell_validation(i).validation_id Then
                                    'fitems.lval.Items(j).Checked = True
                                    fitems.lval.Items(j).SubItems(1).Text = "warning"
            ...
            For i = 0 To osheet.bc_om_insightsections.Count - 1
                For j = 0 To osheet.bc_om_insightsections(i).rows.count - 1
                    With osheet.bc_om_insightsections(i).rows(j)
                        ...
                        Next
                        For k = 0 To .warnings.bc_om_cell_validation.count - 1
                            orow.warnings.Add(.warnings.bc_om_cell_validation(k).validation_id)
                        Next
    ...
    Private Sub set_item(ByVal mode As Boolean, ByVal fitem As bc_am_in_tk_item)
        Try
            ...
            'End If
            REM FIL 5.3
            orow.monetary = fitem.ccurrtype.SelectedIndex
            REM =======
            ...
            If Me.Lselsec.SelectedIndex < osheet.bc_om_insightsections.Count Then
                ...
                For i = 0 To osheet.validations_ids.Count - 1
                    If i < fitem.lval.Items.Count Then
                        ...
                        'End If
                        If fitem.lval.Items(i).SubItems(1).Text = "error" Or fitem.lval.Items(i).SubItems(1).Text = "set" Then
                            oval = New bc_om_cell_validation
                            oval.validation_id = (osheet.validations_ids(i))
                            orow.validations.bc_om_cell_validation.Add(oval)
                        End If
                        If fitem.lval.Items(i).SubItems(1).Text = "warning" Then
                            oval = New bc_om_cell_validation
                            oval.validation_id = (osheet.validations_ids(i))
                            orow.warnings.bc_om_cell_validation.Add(oval)
                        End If
                    End If
                Next
                If mode = True Then
    ...
    Private Sub uxSections_Enter(sender As Object, e As EventArgs) Handles uxSections.Enter
